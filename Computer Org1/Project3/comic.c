...
struct Comic_List init_comic_list() {
  ...
  comic_list.count = 0;
  comic_list.list =
      (struct Comic *)malloc(comic_list.size * sizeof(struct Comic));
  return comic_list;
}
...
void add_comic(struct Comic_List *comic_list, struct Comic comic) {
  ...
  if (comic_list->count == comic_list->size) {
    ...
    comic_list->size *= 2;
    comic_list->list =
        realloc(comic_list->list, comic_list->size * sizeof(struct Comic));
  }
...
struct Comic create_comic(char *date, char *code, char *publisher, char *title,
                          char *cost) {
  struct Comic comic;
...
struct Comic parse_csv_data(FILE *ptr) {
  ...
  fscanf(ptr, "%[^,],%[^,],%[^,],%[^,],%[^\n]\n", date_buffer, code_buffer,
         publisher_buffer, title_buffer, cost_buffer);
  // reads in the data from the file and stores it in the buffers

  // Allocate memory for the strings dynamically and copy the data from the
  // buffer
  ...
  char *publisher =
      (char *)calloc((strlen(publisher_buffer) + 1), sizeof(char));
