...
\begin{proof}
    ...
    Then we create the homotopy $H:[0,2\pi]\times [0,1]$ given by $H(\theta,s)=f(\gamma (\theta))(1-s)+s\cdot \vec p_2$ for all $\theta \in [0,2\pi]$ and $s\in [0,1]$. 

    We have $H(\theta,0)=f(\gamma (\theta))+0\cdot \vec p_2=f(\gamma(\theta))$, and $H(\theta,1)=f(\gamma(\theta ))\cdot 0 + 1\cdot \vec p_2=g(\gamma (\theta))$. 

    Lastly to show that $H$ is continuous.
