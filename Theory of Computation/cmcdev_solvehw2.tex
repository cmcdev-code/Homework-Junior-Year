...
Consider the language $L_2$ from problem $1$ we have that $L_2$ is not regular. However consider the language $L_2^\prime=\{w\in \{0,1\}^*\}$ we have that $L_2^\prime$ is regular as it is recognized by the FA. \begin{center}
    \begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto]
        \node[state,initial, accepting] (q_0)   {$q_0$};
        \path[->]
