...
I wil use the M reduction $DHP\leq_M L$. I will do this by creating the function sending $x\to P_x$ where $P_x$ is the program:
...
\textbf{Input} $y$
\\
\textbf{if} If $x$ halts on input $x$
\\
\textbf{    then} if $y$ is even
\\
\textbf{    then} accept
\\
\textbf{    else} reject
