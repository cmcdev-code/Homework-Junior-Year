...
Now to show that this is a bijection I will come up with an inverse. Let $H^{-1}: \mathcal P(\Sigma^\star)\to F^\prime$ where for $A \in \mathcal P(\Sigma^\star)$ we have $H^{-1}(A)=f$ where for all $w\in \Sigma^\star$ we have $f(w)=1$ if and only if $w\in A$. 
