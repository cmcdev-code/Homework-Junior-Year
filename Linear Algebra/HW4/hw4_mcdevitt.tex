...
\begin{proof}
  ...
  Now assume for $\alpha_1,...,\alpha_n\in \K$ we have the map $Z\in \mathcal (V,\K)$ where
  \[
...
\begin{proof}
    ...
    \end{bmatrix})=1$ and the $\Span(\begin{bmatrix}
        1\\1\\-1
    \end{bmatrix})$ is a subspace of $\text{null} A_n$ then we have $3 \geq \text{nullity}A_n\geq 1$ by the fundamental theorem of linear algebra we get $\text{rank}A_n = \dim \mathbb{R}^3- \text{nullity}A_n\leq 3 -1 $ as the $n$ was an arbitrary natural number then the claim is proved.
