...
\vskip1em
\begin{proof}
    Denote the basis for $V$ as $S,T$ are diagonalizable we have the existence of an eigenvbasis $\{v_1,...,v_n\}$ additionally as $S,T$ have the same eigenvectors we have for any $v_i\in \{v_1,...,v_n\}$ that $S\circ T v_i=S\lambda_i v_i$ where $\lambda_i$ is the eigenvalue corresponding to $v_i$ with respect to $T$. And by the linearity of $S,T$ we get $S\lambda_i v_i = \lambda_i S v_i=\lambda_i (\lambda^\prime_iv_i)$  where $\lambda^\prime_i$ corresponds to the eigenvalue of $v_i$ with respect to $S$. As $\lambda_i, \lambda_i^\prime$ are just scalars we get $\lambda_i^\prime \lambda_i v_i= \lambda_i \lambda^\prime_iv_i=T \lambda_i^\prime v_i=T\circ S v_i$
\end{proof}
